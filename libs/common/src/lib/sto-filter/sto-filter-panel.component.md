...
<sto-filter-panel [expandable]="true" (toggled)="toggleService.isToggled.emit($event)">
  ...
  <sto-filter-table-actions>
    <button matButton="filled">New<mat-icon>arrow_drop_down</mat-icon></button>
  </sto-filter-table-actions>
  <sto-filter-actions>
...
```html
<sto-filter-panel [expandable]="true"></sto-filter-panel>
```
...
```html
<sto-filter-panel [expanded]="true"></sto-filter-panel>
```
...
```html
<sto-filter-panel (toggled)="onToggle($event)"></sto-filter-panel>
```
