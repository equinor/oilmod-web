          ...
          *ngFor="let group of temp; let i = index; trackBy: rowTrackingFn;"
          ...
          <sto-datatable-body-row
            *ngIf="!groupedRows; else groupedRowsTemplate"
            ...
            [offsetX]="offsetX"
            [columns]="columns"
            (rowContextMenu)="cellContextmenu.emit($event)"
...
export class StoDataTableBodyComponent extends DataTableBodyComponent {
  ...
  @Output() cellContextmenu = new EventEmitter<{ event: MouseEvent, row: any, column?: any, value?: any, type?: any }>(false);
  ...
  getRowSummaryStyle() {
    ...
    const colWidth = ( this.columns.map(c => c.width).reduce((r, s) => r + s, 0) );
    if ( colWidth > width ) {
      if ( !isCompactMode ) {
        scroll = 14; // maaagic
      } else {
