...
export class StoDaterangeComponent implements ControlValueAccessor, OnInit, AfterViewInit, OnDestroy {
  ...
  // Use to offset transformed positions
  @Input()
  offsetX = 0;
  @Input()
  offsetY = 0;
   ...
   */
  @Input() inputStyleClass: string[] = [ 'sto-form__field' ];
  /**
  ...
  private updateInputfield(values) {
    if ( values ) {
      const start = format(values.start, this.dateFormat);
      const end = format(values.end, this.dateFormat);
